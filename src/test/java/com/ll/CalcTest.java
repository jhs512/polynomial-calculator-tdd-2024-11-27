...
class CalcTest {
    ...
    void 수식_입력(){
        ...
        assertThat(Calc.run("100 * (2 + 3) / 5 - 20")).isEqualTo(80);
        assertThat(Calc.run("(-1)")).isEqualTo(-1);
        assertThat(Calc.run("((15 / (7 - (1+1))) * -3) - (2 + (1+1))")).isEqualTo(-13);

        String output = outputStream.toString();
        assertThat(output).contains("입력된 수식 : " + "((3 + 5) * 5 + -10) + 10 / 5");
    }
    ...
    void 수식_체크_올바른_경우(){
        int result = Calc.run("((3 + 5) * 5 + -10) + 10 / 5");
        String output = outputStream.toString();
        assertThat(output).doesNotContain("잘못된 수식입니다.");
        assertThat(output).contains("입력된 수식 : ((3 + 5) * 5 + -10) + 10 / 5");
    }
    ...
    void 수식_체크_잘못된_경우(){
        int result = Calc.run("((3 + 5))) * 5 + -10) * 10 / 5");
        String output = outputStream.toString();
        assertThat(output).contains("잘못된 수식입니다.");
    }
